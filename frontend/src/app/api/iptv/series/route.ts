...
export async function GET(request: Request) {
  try {
    ...
    const fetchThread = async (threadId: number) => {
      ...
      for (let i = startBatch; i < endBatch; i++) {
        ...
        let query = supabase
          .from('iptv')
          .select(
            'nome, categoria, temporada, episodio, logo_url, backdrop_url, visualizacoes, created_at, updated_at'
          )
          .eq('tipo', 'serie')
          .eq('is_active', true)
    ...
    // Agrupar por nome de série (remover episódios duplicados)
    const seriesMap = new Map<string, any>();

    allRecords.forEach((record) => {
      if (!seriesMap.has(record.nome)) {
        seriesMap.set(record.nome, {
          nome: record.nome,
      ...
      const serie = seriesMap.get(record.nome)!;

      // Tentar extrair temporada e episódio do nome se não estiverem definidos
      let temporada = record.temporada;
      let episodio = record.episodio;

      if ((temporada === null || temporada === undefined) && record.nome) {
        const parsed = parseSeasonEpisode(record.nome);
        if (parsed) {
          ...
          // Usar valores padrão
          temporada = 1;
          console.warn(`⚠️ Temporada não encontrada para: ${record.nome}`);
        }
      }
