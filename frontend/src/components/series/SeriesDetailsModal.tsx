...
export default function SeriesDetailsModal({
  ...
  const loadSeriesData = async () => {
    if (!series || !series.nome) return;
    
    // Buscar da API TMDB em tempo real
    setLoading(true);
    try {
      const { cleanName, year } = extractYear(series.nome || '');
      ...
      if (data) {
        ...
        setTmdbData({
          name: series.tmdb_title || series.nome,
          overview: series.tmdb_overview,
          poster_path: series.tmdb_poster_path,
          backdrop_path: series.tmdb_backdrop_path,
          first_air_date: series.tmdb_release_date,
          genres: series.tmdb_genres || [],
          vote_average: series.tmdb_vote_average || series.avaliacao,
          number_of_seasons: series.tmdb_number_of_seasons,
          number_of_episodes: series.tmdb_number_of_episodes,
          created_by: series.tmdb_created_by || []
        });
      }
    } catch (error) {
  ...
  const handlePlay = async () => {
    ...
    if (series?.stream_url) {
      setStreamUrl(series.stream_url);
      setShowPlayer(true);
      return;
    }
    ...
    try {
      ...
      const cachedStream = await optimizedCache.getStream(series!.id);
      ...
      const response = await fetch(`/api/iptv/series/${series!.id}/stream`);
      ...
      if (data.url_stream) {
        ...
        alert('Stream não disponível para esta série');
      }
    } catch (error) {
  ...
  const posterUrl = tmdbData?.poster_path 
    ...
    : getPosterUrl(series, 'w500');

  const backdropUrl = tmdbData?.backdrop_path
    ? getTMDBImageUrl(tmdbData.backdrop_path, 'original')
    : getBackdropUrl(series, 'original');

  const trailerUrl = tmdbData?.videos?.[0]
    ? `https://www.youtube.com/embed/${tmdbData.videos[0].key}?autoplay=1`
    : null;
  
  const title = tmdbData?.name || series.tmdb_title || series.nome;
  const overview = tmdbData?.overview || series.tmdb_overview;
  const firstAirDate = tmdbData?.first_air_date || series.tmdb_release_date;
  const voteAverage = tmdbData?.vote_average || series.tmdb_vote_average || series.avaliacao;
  const numberOfSeasons = tmdbData?.number_of_seasons || series.tmdb_number_of_seasons;
  const numberOfEpisodes = tmdbData?.number_of_episodes || series.tmdb_number_of_episodes;
  const createdBy = tmdbData?.created_by || series.tmdb_created_by || [];
  const genres = tmdbData?.genres || series.tmdb_genres || [];
                ...
                <div className="flex-1 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {createdBy && createdBy.length > 0 && (
                      <div>
                        <span className="text-netflix-dimGray">Criado por:</span>
                    ...
                    {firstAirDate && (
                      <div>
                        <span className="text-netflix-dimGray">Primeira exibição:</span>
                        <p className="text-white">
                          {new Date(firstAirDate).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                    )}
                    {numberOfSeasons && (
                      <div>
                        ...
                        <p className="text-white">{numberOfSeasons}</p>
                      </div>
                    )}
                    {numberOfEpisodes && (
                      <div>
                        <span className="text-netflix-dimGray">Episódios:</span>
                        <p className="text-white">{numberOfEpisodes}</p>
                      </div>
                    )}
                    {genres && genres.length > 0 && (
                      <div className="col-span-2">
                        <span className="text-netflix-dimGray">Gêneros:</span>
                        <p className="text-white">
